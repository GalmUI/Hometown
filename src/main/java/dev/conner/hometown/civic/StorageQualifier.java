...
/** Explicit, loaded-only Storage validation. Never opens inventories, persists state, or requests chunks. */
public final class StorageQualifier {
    public enum Reason {
        QUALIFIED,
        ROOM_NOT_FOUND,
        ROOM_AMBIGUOUS,
    ...
    public record Result(boolean qualified, Reason reason, RoomGeometry room,
                         int usableFloorPositions, int storageBlocks, int darkFloorPositions) {
        public Result {
        ...
        public static Result failed(Reason reason) {
            return new Result(false, reason, null, 0, 0, 0);
        }
    }

    private StorageQualifier() {}
    ...
    public static Result qualify(ServerLevel level, Settlement town, BlockPos markerPosition) {
        ...
        // Match the accepted Town Hall marker anchoring rule: signs may be inside the room or on the
        // exterior face of a one-block-thick wall. Never guess when the marker resolves to multiple rooms.
        ...
        for (BlockPos feet : room.interior()) {
            ...
            if (feetState == null || headState == null || floorState == null) {
                return Result.failed(Reason.ROOM_INCOMPLETE);
            }
            if (!feetState.getCollisionShape(world, feet).isEmpty()
                    || !headState.getCollisionShape(world, head).isEmpty()) continue;
            if (!floorState.isFaceSturdy(world, floor, Direction.UP)) continue;
            usable++;
            if (level.getBrightness(LightLayer.BLOCK, feet) < StorageRules.MIN_BLOCK_LIGHT) dark++;
        }

        if (usable < StorageRules.MIN_USABLE_FLOOR_POSITIONS) {
            return new Result(false, Reason.NO_USABLE_FLOOR, room, usable, storage, dark);
        }
        if (storage < StorageRules.MIN_STORAGE_BLOCKS) {
            return new Result(false, Reason.NOT_ENOUGH_STORAGE, room, usable, storage, dark);
        }
        if (dark > 0) {
            return new Result(false, Reason.UNSAFE_LIGHTING, room, usable, storage, dark);
        }
        return new Result(true, Reason.QUALIFIED, room, usable, storage, dark);
    }
